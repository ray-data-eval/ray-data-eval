#!/usr/bin/env bash
# Restore the vanilla Ray installation, undoing scripts/install_ray_data.sh.
#
...
BACKUP=$(find "$SITE_RAY" -maxdepth 1 -name 'data.stock-*' -type d | head -1)

if [ -z "$BACKUP" ]; then
  echo "No stock backup found in $SITE_RAY."
...
leftover = [k for k in ("scheduling_policy", "microbatch_size", "llf_latency_target")
            if hasattr(ctx, k)]
print("  Ray Data knobs still present:", leftover or "none — stock Ray restored")
PY
