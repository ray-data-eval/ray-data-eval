...
#
# Both need a JVM and so run out-of-process. They are installed into
...
pip install --quiet "pyspark==${SPARK_VERSION}" "apache-flink==${FLINK_VERSION}" \
  "tensorflow==2.16.1" || {
    echo
    echo "  pip hit a conflict. The baselines run out-of-process, so a" >&2
...
echo
echo "Then, e.g.:"
echo "  python experiments/baselines/spark_fig09.py --scale paper --sweep"
echo
